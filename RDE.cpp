...
void localkdb::classify(const instance& inst, std::vector<double> &posteriorDist)
{
    ...
    for (unsigned int x1 = 0; x1 < noCatAtts_; x1++)
    {
        for (CatValue y = 0; y < noClasses_; y++)
        {
            ...
            } else if (parents_1[x1].size() == 1)
            {
                ...
                if (dist_1.xxyCounts.xyCounts.p(x1, inst.getCatVal(x1), y) > dist_1.xxyCounts.p(x1, inst.getCatVal(x1), parents_1[x1][0], inst.getCatVal(parents_1[x1][0]), y))
                {
            ...
            } else if (parents_1[x1].size() == 2)
            {
                ...
                if (dist_1.xxyCounts.p(x1, inst.getCatVal(x1), parents_1[x1][0], inst.getCatVal(parents_1[x1][0]), y) > dist_1.xxyCounts.p(x1, inst.getCatVal(x1), parents_1[x1][0], inst.getCatVal(parents_1[x1][0]), y))
                {
    ...
    //条件互信息的加和结果
     /*std::vector<double> summi;
     getcomdmutualin_locorder(dist_1.xxyCounts, summi,  inst);
    std::vector<ClassLabel> originRank(noClasses_);
    for (CatValue i = 0; i < noClasses_; i++)
    {
        originRank[i].classLabel = i;
        originRank[i].probability = posteriorDist[i];
        originRank[i].sum=summi[i];
    }
    std::sort(originRank.begin(), originRank.end());
    
    for (CatValue i = 0; i < noClasses_; i++)
    {
        cout<<"类标签: "<<originRank[i].classLabel<<"  后验概率: "<<originRank[i].probability <<"  条件互信息和: "<< originRank[i].sum<<endl;
    }
    cout<<"真实的类标签:"<<inst.getClass()<<endl;
    cout<<endl;*/
    ...
    if (union_kdb_localkdb == true){
        ...
        //unsigned int i = inst.getClass();
        //cout<<i<<' '<<posteriorDist[i]<<endl;
    }

}

void localkdb::classify_General(const instance& inst, std::vector<double> &posteriorDist1)
{
    //    printf("classify_General\n");
    //全局parents_的联合概率
    for (CatValue y = 0; y < noClasses_; y++)
    {
        posteriorDist1[y] = classDist_.p(y) * (std::numeric_limits<double>::max() / 2.0);
...
void localkdb::classify_Local(const instance& inst, std::vector<double> &posteriorDist)
{
